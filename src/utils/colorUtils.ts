 ...
 */
const createGradientColorRegex = (color: HexColor, caseSensitive = false): RegExp => {
  const escapedColor = color.replace('#', '\\#');
  const flags = caseSensitive ? 'g' : 'gi';
...
export const replaceAllColors = (
  ...
  try {
    ...
    if (colors.primary) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.primary, colors.primary, options);
      replacedColors[DEFAULT_COLORS.primary] = colors.primary;
    }

    if (colors.secondary) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.secondary, colors.secondary, options);
      replacedColors[DEFAULT_COLORS.secondary] = colors.secondary;
    }

    if (colors.accent) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.accent, colors.accent, options);
      replacedColors[DEFAULT_COLORS.accent] = colors.accent;
    }

    if (colors.neutral) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.neutral, colors.neutral, options);
      replacedColors[DEFAULT_COLORS.neutral] = colors.neutral;
    }

    if (colors.special) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.special, colors.special, options);
      replacedColors[DEFAULT_COLORS.special] = colors.special;
    }

    // Замена градиентных цветов
    if (colors.gradientStart) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.gradientStart, colors.gradientStart, options);
      replacedColors[DEFAULT_COLORS.gradientStart] = colors.gradientStart;
    }

    if (colors.gradientEnd) {
      modifiedSvg = replaceColorInSvg(modifiedSvg, DEFAULT_COLORS.gradientEnd, colors.gradientEnd, options);
    ...
    if (options.customMappings) {
      Object.entries(options.customMappings).forEach(([oldColor, newColor]) => {
        try {
          modifiedSvg = replaceColorInSvg(modifiedSvg, oldColor, newColor, options);
 ...
 */
export const getColorContrast = (color1: HexColor, color2: HexColor): number => {
 ...
 */
export const hexToRgb = (hex: HexColor): { r: number; g: number; b: number } | null => {
